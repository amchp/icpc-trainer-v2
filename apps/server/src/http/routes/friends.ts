...
export const friendRoutes = HttpRouter.empty.pipe(
  HttpRouter.get(
    "/api/friends",
    Effect.gen(function* () {
      const rosterService = yield* RosterService;
      return yield* schemaJson(RosterResponse, yield* rosterService.getRoster("friend"));
    }).pipe(
  ...
  HttpRouter.put(
    ...
    Effect.gen(function* () {
      ...
      return yield* schemaJson(
        RosterResponse,
        yield* rosterService.replaceRoster("friend", payload.handles),
      );
    }).pipe(
      Effect.catchTag("RosterError", (error) =>
        Effect.succeed(errorJson(rosterStatus(error), error._tag, error.message)),
  ...
  HttpRouter.post(
    ...
    Effect.gen(function* () {
      ...
      return yield* schemaJson(
        SyncRosterResponse,
        yield* rosterService.syncRoster("friend", payload.force, payload.handles),
      );
    }).pipe(
      Effect.catchTag("RosterError", (error) =>
        Effect.succeed(errorJson(rosterStatus(error), error._tag, error.message)),
